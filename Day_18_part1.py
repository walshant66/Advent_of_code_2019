...
def distanceToCollectKeys(currentKey, Graph_A, cache=None,keys_collected=None): 
	...
	keys_all=[x[1] for x in Graph_A.path_dict_tmp.keys() if '@' in x[0]]
	if keys_collected!=[]:
	...
	reachable_keys_1=get_all_reachable_keys(Graph_A,currentKey,keys_collected)
	for key in reachable_keys_1:
		[new_distance,cache]=distanceToCollectKeys(key, Graph_A, cache,keys_collected)
		d = len(Graph_A.path_dict_tmp[currentKey+ key]) - 1 + new_distance
		result = min([result, d])
	if cache==None:
		cache={}
...
class LearningCase(unittest.TestCase):
	def test_starting_out(self):
		...
		A2_tmp=Graph(get_graph_dict_all(enumerate_empty_positions(get_maze("test_2.txt"))))
		A2_tmp.path_dict_tmp=get_all_keys_distances(A2_tmp)
		A3_tmp=Graph(get_graph_dict_all(enumerate_empty_positions(get_maze("test_3.txt"))))
		A3_tmp.path_dict_tmp=get_all_keys_distances(A3_tmp)
		A4_tmp=Graph(get_graph_dict_all(enumerate_empty_positions(get_maze("test_4.txt"))))
		A4_tmp.path_dict_tmp=get_all_keys_distances(A4_tmp)
		A5_tmp=Graph(get_graph_dict_all(enumerate_empty_positions(get_maze("test_5.txt"))))
		A5_tmp.path_dict_tmp=get_all_keys_distances(A5_tmp)
		A6_tmp=Graph(get_graph_dict_all(enumerate_empty_positions(get_maze("test_6.txt"))))
		A6_tmp.path_dict_tmp=get_all_keys_distances(A6_tmp)
		puzzle_tmp=Graph(get_graph_dict_all(enumerate_empty_positions(get_maze("test_puzzle.txt"))))
		puzzle_tmp.path_dict_tmp=get_all_keys_distances(puzzle_tmp)
		
		self.assertEqual(distanceToCollectKeys('@', A2_tmp),8)
		self.assertEqual(distanceToCollectKeys('@', A3_tmp),86)
		self.assertEqual(distanceToCollectKeys('@', A4_tmp),132)
		self.assertEqual(distanceToCollectKeys('@', A5_tmp),136)
		self.assertEqual(distanceToCollectKeys('@', A6_tmp),81)
		self.assertEqual(distanceToCollectKeys('@', puzzle_tmp),5402)
